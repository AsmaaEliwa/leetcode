...
//
func sumTwoStrInt(_ strNum:String)->Int{
    var sum = 0
    for i in strNum {
//        print(i)
        sum+=Int(String(i)) ?? 0
...
//sumTwoStrInt("33")   // 6
func isSymetric(_ num:Int)->Bool{
    var numAsStr = String(num)
    var leftSlice:String = ""
    var rightSlice:String = ""
    if numAsStr.count % 2 == 0 {
        for (i,char) in numAsStr.enumerated(){
            if i < numAsStr.count/2 {
                leftSlice.append(char)
            }else {
                rightSlice.append(char)
            }
        }
        if sumTwoStrInt(leftSlice) == sumTwoStrInt(rightSlice){
            return true
        }
    
    }
  
//   print(rightSlice , leftSlice)
    return false
}
//isSymetric(3324)
